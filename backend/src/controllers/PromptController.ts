...
export const remove = async (
  ...
  try {
    ...
    if (count > 0) return res.status(200).json({ message: "No se pudo eliminar, verique que este prompt no está siendo usado en alguna conexión Whatsapp!" });
    ...
    return res.status(500).json({ message: "No se pudo eliminar, verique que este prompt no está siendo usado!" });
  }
};
